...
def customer_detail_page(customer_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    enquiries = db.query(Enquiry).filter(Enquiry.user_id == customer_id).order_by(Enquiry.id.desc()).all()
    orders = []
    ...
    return templates.TemplateResponse(request=request, name="admin/customer_detail.html", context={"customer": customer, "enquiries": enquiries, "orders": orders})


@router.get("/orders")
def admin_orders_page(request: Request, db: Session = Depends(get_db)):
    orders_enabled = inspect(db.bind).has_table("orders")
    orders = db.query(Order).order_by(Order.created_at.desc()).all() if orders_enabled else []
    return templates.TemplateResponse(request=request, name="admin/orders.html", context={"orders": orders, "orders_enabled": orders_enabled})


@router.get("/settings")
def admin_settings_page(request: Request):
    from app.content.store import load_content
    return templates.TemplateResponse(request=request, name="admin/settings.html", context={"content": load_content()})
...
async def admin_settings_save(request: Request):
    ...
    content.setdefault("site", {})["name"] = str(form.get("name", "Intra Aura")).strip()
    content["site"]["tagline"] = str(form.get("tagline", "")).strip()
    content["site"]["copyright"] = str(form.get("copyright", "")).strip()
    header = content.setdefault("header", {})
    existing = header.get("nav_links", [])
    by_label = {str(x.get("label", "")).strip().lower(): x.get("url", "") for x in existing if isinstance(x, dict)}
    labels = {"home":"Home","products":"Products","categories":"Categories","services":"Services","portfolio":"Portfolio","about":"About Us","contact":"Contact"}
    defaults = {"home":"/","products":"/products","categories":"/categories","services":"/services","portfolio":"/portfolio","about":"/about","contact":"/contact"}
    header["nav_links"] = [{"label": (str(form.get(f"nav_{k}", v)).strip() or v), "url": by_label.get(v.lower(), defaults[k])} for k,v in labels.items()]
